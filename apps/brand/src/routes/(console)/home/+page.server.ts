...
import { brandPath, listBrandCampaigns, listBrandProducts, listBrandRequests, requireBrand } from '$lib/server/brand';
 ...
 * · 진행 중(LIVE) · 확정(SCHEDULE_CONFIRMED) 캠페인 · 상품 현황 카운트 · 브랜드 카드를 만든다. 장부 4 KPI · 등급 카드 · 인플루언서 찾기는 5·6단계.
...
export const load: PageServerLoad = async (event) => {
	...
	const [products, requests, campaigns] = await Promise.all([listBrandProducts(brand.id), listBrandRequests(brand.id), listBrandCampaigns(brand.id)]);

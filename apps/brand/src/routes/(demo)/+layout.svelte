<script lang="ts">
	 ...
	 * 콘솔과 겹치는 경로는 `/demo-*` 로 옮겼다(홈 `/demo` · 로그인 `/demo-login`). 상품 관리 탭은 2단계에서 콘솔 `/products` 로, 주문·발주 · 고객 문의 탭은 4단계에서 콘솔 `/orders` `/cs` 로, 실시간 매출 · 정산 · 마이페이지는 5단계에서 콘솔 `/sales` `/settle` `/my` 로 넘어가 데모에서 뺐다(`go.screen()` 은 콘솔 화면에 닿는다).
	...
	import { S, D_, brand, celBal, dmUnreadN, brandPending, CEL, act, go, setNavigate } from '@sellery/core';

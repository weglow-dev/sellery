 ...
 * (docs/monorepo-migration.md §3.3 "server-only 대체 세 겹" 2번 · apps/influencer/src/lib/server/db.ts 와 동일).
 * 설정 주입(`configureDb`)은 `./env` 가 한다 — 이 배럴을 import 하면 같이 실린다. 브랜드 게이트·가입은 `./brand`.
 */
import './env';

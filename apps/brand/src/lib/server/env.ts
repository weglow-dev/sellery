 ...
 * 토스 시크릿 · `RRN_ENC_KEY` 는 브랜드 콘솔에 없다(docs/brand-console-plan.md §2.1 — 브랜드 결제는 6단계, 주민번호는 브랜드에 없음).
 * Vercel `sellery-brand` 에 필요한 이름: `SUPABASE_SERVICE_ROLE_KEY`(필수) · `SLACK_WEBHOOK_URL`(선택) — docs/deploy.md §1.2.
 */

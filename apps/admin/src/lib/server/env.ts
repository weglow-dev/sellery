 ...
 * (docs/monorepo-migration.md 결정 11 · apps/brand/src/lib/server/env.ts 와 같은 패턴). hooks.server.ts 가 이 모듈을 import 하는 것으로 주입이 끝난다.
 ...
 * 토스 시크릿 · `RRN_ENC_KEY` 는 관리자 콘솔에 아직 없다(결제·주민번호 열람은 이후 단계).
 * Vercel `sellery-admin` 에 필요한 이름: `SUPABASE_SERVICE_ROLE_KEY`(필수) · `SLACK_WEBHOOK_URL`(선택) — docs/deploy.md §1.2.
 */
...
/** 고객 사이트 오리진 ("← 셀러리 고객 사이트"). 형식이 어긋나면 로컬 shop dev 주소. */

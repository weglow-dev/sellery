 ...
 * "정산·돈" PR-B 에서 `/settle` `/orders` 를 뺐고, 파트너 관리에서 `/brands` 를 뺐다 — 실제 `(console)/(money)/{settle,orders}` ·
 * `(console)/brands` 가 그 이름을 쓴다(데모의 `go.screen('settle'|'orders'|'brands')` 는 콘솔 화면에 닿는다 · 브랜드 4·5단계와 같은 규칙).
 * 데모 화면 자체도 같이 지운다 — 콘솔 화면이 그 역할을 대체한다.
 */
export const DEMO_PATHS = ['/demo', '/products', '/influencers', '/match', '/revenue', '/c', '/s'] as const;

<script lang="ts">
	/**
	 * 비밀번호 재설정 요청 — web (public)/password/page.tsx + password-form.tsx 1:1 (프로토타입 login.html fReset · docs/inf-console-plan.md §4.1).
	 * `resetPasswordForEmail` → 메일(token_hash · type=recovery) → `/influencer/auth/confirm` → `/influencer/password/new`.
	...
	async function onSubmit(e: SubmitEvent) {
		...
		const { error: e2 } = await supabase.auth.resetPasswordForEmail(em, {
			redirectTo: `${window.location.origin}${consolePath('seller', '/auth/confirm')}?next=${encodeURIComponent(consolePath('seller', '/password/new'))}`
		});
		busy = false;
...
<div class="console-auth">
	<section class="card static">
		...
		<p class="meta">가입한 이메일을 입력하면 재설정 링크를 보내드려요. 링크는 다른 기기에서 열어도 됩니다.</p>
		{#if done}
		...
		<div class="foot">
			<span></span>
			<a href={consolePath('seller', '/login')}>로그인으로</a>
		</div>
	</section>
</div>

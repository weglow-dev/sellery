<script lang="ts">
	/**
	 * 새 비밀번호 폼 — web password/new/new-password-form.tsx 1:1. `updateUser({ password })` 뒤 콘솔 홈으로 전체 이동.
	...
	import { PASSWORD_RE } from '@sellery/db/partner/signup-rules';
	...
	async function onSubmit(e: SubmitEvent) {
		...
		window.location.assign(consolePath('seller', '/home'));
	}
</script>
	...
	<section class="card static">
		...
		{:else}
			...
			<div class="btnrow" style="margin-top:14px">
				<a href={consolePath('seller', '/password')} class="btn pri sm">재설정 메일 다시 받기</a>
				<a href={consolePath('seller', '/login')} class="btn ghost sm">로그인으로</a>
			</div>
		{/if}
	</section>
</div>

<script lang="ts">
	/**
	 * 인증 메일 안내 + [메일 다시 보내기] — web verify-sent/page.tsx + resend-button.tsx 1:1.
	 * `supabase.auth.resend({ type:'signup' })` · 60초 쿨다운(Supabase 도 같은 주소로 60초에 1회만 보낸다). 존재하지 않는 이메일이어도 결과를 구분해 보여 주지 않는다.
	 * `emailRedirectTo` 는 가입 폼과 같은 콘솔 전용 `/influencer/auth/confirm?next=/influencer/home`(결정 15).
	 */
	...
	async function resend() {
		...
		const { error } = await supabase.auth.resend({
			type: 'signup',
			email: data.email,
			options: { emailRedirectTo: `${window.location.origin}${consolePath('seller', '/auth/confirm')}?next=${encodeURIComponent(consolePath('seller', '/home'))}` }
		});
		busy = false;
		startCooldown();
...
<div class="console-auth">
	<section class="card static">
		...
		<p class="meta">
			{#if data.email}<b>{data.email}</b> 로 인증 링크를 보냈습니다.{:else}가입한 이메일로 인증 링크를 보냈습니다.{/if}

...
import { checkoutHref, EMPTY_DRAFT, parseCheckoutParams, type ShippingDraft } from '@sellery/payments/checkout-rules';
import { fetchCampaignCard } from '$lib/server/db';

/**
 * /checkout?c={code}&o={optIdx}&q={qty} — 서버 검증 후 CheckoutClient (app-plan §6.1 · ux-spec §3.4 · web checkout/page.tsx 1:1 · docs/monorepo-migration.md §4.1).
...
export const load: PageServerLoad = async (event) => {
	...
	if (!user) redirect(303, `/login?next=${encodeURIComponent(checkoutHref(code, optionIndex, qty))}`);
	...
	if (!buyable.ok) return { blocked: buyable.message, storeUrl, checkout: null };
	...
	return {
		blocked: null,
		storeUrl,
		checkout: { card, optionIndex, qty, customerKey: user.id, email: user.email ?? null, defaults }
	};
};

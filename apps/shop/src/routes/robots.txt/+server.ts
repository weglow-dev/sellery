 ...
 * 판매 링크 `/s/` · `/c/` 와 홈, 셀러리 소개(`/about`)·인플루언서 목록(`/influencers`), 이용약관·개인정보처리방침은 색인 허용, 개인화·결제·인증·고객 문의(`/cs`)·API 경로와 리라이트되는 콘솔·데모(`/influencer` `/brand` `/admin`)는 제외.
...
const RULES = {
	...
	disallow: ['/api/', '/checkout', '/account', '/cs', '/login', '/auth/', '/influencer', '/brand', '/admin']
};

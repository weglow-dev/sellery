...
import { GUEST_CHECKOUT_COOKIE, GUEST_CHECKOUT_COOKIE_MAX_AGE, parseGuestBody, type GuestBuyer } from '@sellery/db/guest-order';
...
import { createAdminClient, createGuestCustomer, ensureCustomer, fetchCampaignCard, rateLimit, type Admin } from '$lib/server/db';
 ...
 *
 * 입력 { code, optionIndex, qty, recipient, phone, postcode, address1, address2?, memo?, saveAddress?, guest?:{ name, phone, email?, consent:true } }
 * 순서(§6.2 ①~⑦):
...
export const POST: RequestHandler = async (event) => {
	...
	// ① 로그인 또는 비회원 주문자 정보 (0021). 로그인 상태면 guest 본문은 무시한다(회원 주문).
	const { user } = await locals.safeGetSession();
	let guest: GuestBuyer | null = null;
	if (!user) {
		if (body.guest === undefined || body.guest === null) return apiError(401, 'UNAUTHORIZED');
		guest = parseGuestBody(body.guest);
		if (!guest) return apiError(400, 'BAD_REQUEST', '주문자 이름·연락처와 개인정보 동의를 확인해주세요');
		if (!rateLimit(`checkout:ip:${event.getClientAddress()}`, MAX_SESSIONS_PER_30M)) {
	...
	const saveAddress = body.saveAddress === true && !!user;
	...
	// 사용자당 미결제 세션 상한 (§5.1): 최근 30분 20건 초과 → 429 (비회원은 위 IP 레이트리밋)
	if (user) {
	...
	// ⑤ customers 행 보장 (필수 — customer_id 를 세션에 저장). 비회원은 새 guest 행(0021).
	let customerId: string;
	try {
		customerId = user ? (await ensureCustomer(admin, user)).id : await createGuestCustomer(guest!, admin);
	} catch (e) {
	...
	const { data: session, error: insertError } = await admin
		...
		.insert({
			...
			buyer_name: user ? displayName(user) : guest!.name,
			buyer_phone: user ? phone : guest!.phone,
			buyer_email: user ? (user.email ?? null) : guest!.email,
	...
	// 비회원: 세션 소유 쿠키 (HttpOnly · 45분) — /api/payments/confirm 이 pre.id 와 대조한다
	if (!user) {
		cookies.set(GUEST_CHECKOUT_COOKIE, session.id, { httpOnly: true, sameSite: 'lax', secure: !dev, path: '/', maxAge: GUEST_CHECKOUT_COOKIE_MAX_AGE });
	}
	...
	return json({
		...
		customerKey: user?.id ?? customerId,
		phone: user ? phone : guest!.phone,
		email: user ? (user.email ?? null) : guest!.email
	});
};

...
/**
 * POST /api/payments/webhook — 토스 웹훅 (app-plan §6.2 · §7.3 · web api/payments/webhook/route.ts 1:1).
 *
 ...
 * JSON 본문이라 SvelteKit 내장 CSRF 검사 대상이 아니다(form 콘텐츠 타입만 검사) — §4.2.
 */
...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (paymentKeyRaw !== undefined && paymentKeyRaw !== null && (bodyPaymentKey === null || !TOSS_KEY_RE.test(bodyPaymentKey))) {
		return bad();
	}
	...
	if (!session && bodyPaymentKey) {
		const { data: s } = await admin.from('checkout_sessions').select(SESSION_LITE_COLS).eq('payment_key', bodyPaymentKey).maybeSingle();
		session = (s as SessionLite | null) ?? null;
		if (!session) {
	...
	if (!look.ok || isTossError(look.body)) {
		if (isUncertain(look)) {
			...
			return json({ ok: false, code: 'LOOKUP_FAILED' }, { status: 502 });
		}

<script lang="ts">
	/**
	 * 주문 상세 — web account/orders/[code]/page.tsx 1:1: 주문 상품 · 주문 정보 · 배송 정보(service 조인이라 shipping 원문) · 교환/환불.
	 * 환불 정책 원문(js/60-customer.js L282): "판매 종료 후 {n}일 동안 교환·환불 신청 가능 (단순 변심 7일 · 하자 {n}일)".
	 * 환불 성공 → `invalidateAll()` (router.refresh 대체) → 칩 '환불 완료'.
	 */
	...
	import { CsModalButton, ProductIcon, RefundButton, ShipInfo, StatusChip } from '@sellery/ui/site';
...
<div class="store">
	...
	<h2 class="pg">주문 상세 <small>{CODE} · {md(order.paid_at)} 주문</small></h2>
	...
	<div class="card static">
		...
		{:else if refund.ok}
			<div class="btnrow" style="align-items:center;margin-bottom:12px">
				<RefundButton class="sm" code={order.code} productName={order.product.name} optionName={order.option_name} qty={order.qty} amount={order.amount} onRefreshed={refresh} />

...
//
//   cd web && node scripts/dev-seller.mjs --email dev-seller@sellery.test --seller s1 [--password sellery2026]
//
...
// - .env.local 은 자동으로 읽는다(값 미출력). 비밀번호는 --password 또는 기본값 sellery2026(프로토타입 데모 버튼과 동일) — 키·시크릿은 출력하지 않는다.
...
if (!email || !email.includes("@") || !sellerRef) {
  console.error("usage: node scripts/dev-seller.mjs --email <email> --seller <s1|uuid> [--password <pw>]");
  process.exit(2);
}
...
if (!url || !serviceKey) {
  console.error("[dev-seller] NEXT_PUBLIC_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY 가 없습니다 — web/.env.local 을 확인하세요.");
  process.exit(1);
}

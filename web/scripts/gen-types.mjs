// `npm run gen:types` — Supabase 타입 생성 (Windows/리눅스 공용).
//
// 저장소 루트(supabase/config.toml 이 있는 곳)에서 `npx supabase gen types typescript --linked` 를 실행하고
// 결과를 web/src/lib/database.types.ts 에 UTF-8(BOM 없음) · LF 로 쓴다.
...
const outFile = resolve(webDir, "src", "lib", "database.types.ts");

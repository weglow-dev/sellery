/**
 * 체크아웃 순수 규칙 — URL 파라미터 · 배송지 검증 · 성공/실패 페이지 문구 매핑. 소유: D.
 * React·Next 를 import 하지 않는다 (서버 컴포넌트·클라이언트·node 단위 확인 모두에서 쓴다).
 *
 * 원문 출처: ux-spec §3.4(배송 정보 검증 문구) · §3.5(실패 사유) · app-plan §6.3(성공 페이지 code → 문구 표 — 원본).
 */
import { CAMPAIGN_CODE_RE } from "@/lib/campaign";
...
/** `/checkout?c=&o=&q=` (C 의 store/buy-cta.tsx checkoutUrl 과 같은 모양 — 서버에서도 쓰기 위해 별도 정의) */

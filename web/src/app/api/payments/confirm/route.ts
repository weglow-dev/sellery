...
/**
 * POST /api/payments/confirm — 토스 승인 (app-plan §6.2 · §7.1 · §7.2). 소유: E.
 *
  ...
  try {
    const [{ data: s }, { data: c }] = await Promise.all([
      admin
        ...
        .select("option_name, qty, amount, buyer_name")
        .eq("id", session.id)
        .maybeSingle(),
      admin
        .from("campaigns")
        .select("code, products(name, emoji, thumb_url), sellers(name, handle), brands(name)")
...
async function fillCustomerFromSession(admin: Admin, session: SessionLite): Promise<void> {
  try {
    const { data: s } = await admin
      .from("checkout_sessions")
      .select("customer_id, buyer_phone, shipping")
      .eq("id", session.id)
      .maybeSingle();
...
export async function POST(request: Request) {
  ...
  if (!claim.claimed) {
    ...
    return NextResponse.json({ ok: true, orderCode: r.orderCode, already: true, card });
  }
  ...
  const card = await buildSuccessCard(admin, session);
  return NextResponse.json({ ok: true, orderCode: r.orderCode, already: r.already || undefined, card });
}

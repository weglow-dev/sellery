...
import { LINKCTX_COOKIE, LINK_CODE_RE } from "@/lib/linkctx";
import { cleanText, normalizePhone } from "@/lib/text";
import { generateOrderId } from "@/lib/money";
import { apiError, gateIsLive, rejectCrossSite, softStockLeft, type Admin, type CampaignGate } from "@/lib/checkout-sync";
import type { Shipping } from "@/lib/types";
import type { Json } from "@/lib/database.types";

/**
 * POST /api/checkout — 결제 세션 생성 (app-plan §6.2 · §7.1). 소유: E.
 *
...
export async function POST(request: Request) {
  ...
  const rawLink = cookieStore.get(LINKCTX_COOKIE)?.value ?? "";

...
/**
 * GET|POST /api/cron/reconcile — CONFIRMING 고착·CANCEL_PENDING 종결 (app-plan §7.5). 소유: E.
 *
 * 인증: `Authorization: Bearer ${CRON_SECRET}` (Vercel Cron 규약) 또는 `x-cron-secret` 헤더. CRON_SECRET 미설정이면 503(실행 안 함).
 * 슬라이스 1 은 수동 호출(curl / 로컬 스크립트); 슬라이스 4 에서 Vercel Cron 에 올린다(§12).
 *
 * 순서:
 ...
 *      (CONFIRMING 의 미승인 분기만 여기서, 나머지는 lib/checkout-sync syncFromPayment 가 웹훅과 공유)
...
async function run(request: Request) {
  ...
  if (auth === null) {
    return NextResponse.json({ ok: false, code: "NOT_CONFIGURED", message: "CRON_SECRET is not set" }, { status: 503 });
  }
  ...
  if (staleError) {
    ...
    return NextResponse.json({ ok: false, code: "DB_ERROR", message: staleError.message, expired }, { status: 500 });
  }

  const sessions = (Array.isArray(staleJson) ? staleJson : [])
    .map(parseSessionLite)
  ...
  return NextResponse.json({ ok: true, expired, checked: results.length, results });
}

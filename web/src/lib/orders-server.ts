...
/**
 * 내 주문 조회 — 계약 docs/app-plan.md §10.0 · §6.1. 소유: F.
 *
 ...
 *
 * 호출자는 반드시 세션의 user.id 를 넘긴다 — 이 모듈은 인증을 하지 않는다 (페이지·라우트가 getSessionUser 로 확인).
 */
import { createAdminClient } from "@/lib/supabase/admin";
import type { OrderStatus, Shipping } from "@/lib/types";
import { DEFAULT_CLEAR_DAYS } from "@/lib/order-status";
...
/** orders.shipping jsonb → Shipping (필수 키가 문자열이면 통과, 아니면 null) */
...
/** 조인이 비었거나(FK 불일치) 상태값이 낯설면 null — 목록에서는 제외, 상세에서는 notFound */
...
/** user_id 불일치·없음·형식 오류 → null (페이지는 notFound()). 화면은 대문자로 보여 주므로 소문자도 같이 찾는다. */
export async function fetchMyOrder(userId: string, code: string): Promise<MyOrder | null> {
  const raw = (code ?? "").trim();
...
export async function fetchOrderSettings(): Promise<{ clear_days: number }> {
  try {
    const admin = createAdminClient();
    const { data, error } = await admin.from("platform_settings").select("value").eq("key", "clear_days").maybeSingle();
...
export async function fetchMyAccount(userId: string): Promise<MyAccount | null> {
  try {
    const admin = createAdminClient();
    const { data, error } = await admin.from("customers").select("email, created_at").eq("user_id", userId).maybeSingle();
    if (error) throw new Error(error.message);

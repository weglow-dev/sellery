...
- 배포: **Vercel** 프로젝트 4개 (team `weglow-team`) — `sellery-shop`(Root Directory `apps/shop` · 도메인 `sellery.life` · `www`) · `sellery-influencer` · `sellery-brand` · `sellery-admin`(각 `apps/<앱>` · "Include source files outside of the Root Directory" ON · Node 22 · `@sveltejs/adapter-vercel`) + `sellery-app`(옛 Next `web/` · 동결 · 롤백 전용 · 2026-09-28 경 삭제). `apps/shop/vercel.json` rewrites 가 `/influencer/*`(→ `sellery-influencer` 콘솔) · `/brand/*` `/admin/*`(→ 데모 프로젝트) 를 프록시 → 브라우저 오리진 하나 · 세션 쿠키 공유. GitHub 앱 연동 → `main` 병합 = 프로덕션, PR = 앱별 미리보기 URL 댓글. **정본 docs/deploy.md**(프로젝트 표 · 환경변수 이름 · 리라이트 표 · 도메인 전환 기록 · 외부 서비스 · DB · 운영 스크립트 · Vercel 주의) · 단계 기록은 docs/monorepo-migration.md §7(S1~S5 완료).

...
- <!-- S4 --> **S4 완료 전에는 정식 주소 https://sellery.life 를 `web/`(Next.js 16 · Vercel `sellery-app` · Root Directory `web`) 가 서비스합니다** — 판매 링크·카카오 로그인·토스 결제·내 주문·약관/처리방침, 인플루언서 콘솔 1~2단계. 도메인을 `sellery-shop` 으로 옮기는 순간([docs/deploy.md](docs/deploy.md) §3 체크리스트) 이 문장을 지웁니다. <!-- /S4 -->
- **S4 뒤 `sellery.life` 는 `apps/shop`(Vercel `sellery-shop`) 이 받습니다** — 고객 사이트 전부(`/` `/s/*` `/c/*` `/login` `/auth/*` `/checkout*` `/account/*` `/api/*` `/terms` `/privacy`, S2~S3 에서 `web/` 과 1:1 이식 · Supabase SSR · 토스페이먼츠). `apps/shop/vercel.json` 리라이트: `/influencer/*` · `/_next/*` → `sellery-app.vercel.app`(**Next 콘솔 · S5 까지 임시**), `/brand/*` `/admin/*` → `sellery-brand` · `sellery-admin`(**localStorage 데모 — 데모 띠 `DemoBanner` · `noindex` · `/brand/robots.txt` `/admin/robots.txt` disallow**, 결정 D). 배포·운영 정본은 [docs/deploy.md](docs/deploy.md); [web/DEPLOY.md](web/DEPLOY.md) 는 `sellery-app` 한정. 설계는 [docs/app-plan.md](docs/app-plan.md) · [docs/inf-console-plan.md](docs/inf-console-plan.md) · [docs/monorepo-migration.md](docs/monorepo-migration.md).
- `apps/influencer` · `apps/brand` · `apps/admin` 은 아직 localStorage 데모입니다(influencer 데모는 S5 전까지 `sellery-influencer.vercel.app` 에서만 보임). **다음 작업은 S5 — 인플루언서 콘솔 1~2단계를 `apps/influencer` 로 이식 → 리라이트를 `sellery-influencer` 로 교체(`/_next/*` 줄 삭제) → `web/` 삭제**, 이어서 `apps/brand` → `apps/admin`. 그 전까지 `web/` 는 손대지 않습니다(버그 수정만).
...
- 배포: **Vercel** 프로젝트 5개 (team `weglow-team`) — `sellery-shop`(Root Directory `apps/shop` · 도메인 `sellery.life` 루트) · `sellery-influencer` · `sellery-brand` · `sellery-admin`(각 `apps/<앱>` · "Include source files outside of the Root Directory" ON · Node 22 · `@sveltejs/adapter-vercel`) + `sellery-app`(Next `web/` · **S5 까지** `/influencer/*` 리라이트 대상 · S5 뒤 1주에 삭제). `apps/shop/vercel.json` rewrites 가 `/influencer/*` `/_next/*`(→ `sellery-app`, S4 임시) · `/brand/*` `/admin/*`(→ 데모 프로젝트) 를 프록시 → 브라우저 오리진 하나 · 세션 쿠키 공유. GitHub 앱 연동 → `main` 병합 = 프로덕션, PR = 앱별 미리보기 URL 댓글. **정본 docs/deploy.md**(프로젝트 표 · 환경변수 이름 · 리라이트 표 · 도메인 전환 체크리스트 · 운영 스크립트) · 단계는 docs/monorepo-migration.md §7. <!-- S4 --> 도메인 이동 전에는 `sellery.life` = `sellery-app`(web/). <!-- /S4 -->

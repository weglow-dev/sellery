...
**4단계 PR-B 적용 기록(2026-09-21)**: 화면 `apps/influencer/src/routes/(console)/pay/{new,[id],success,fail}`(SvelteKit 폼 액션 · **JSON API 라우트 없음** — 토스는 successUrl/failUrl 로 돌아오고 각 서버 load 가 `confirmSamplePurchase`/`recordSampleWidgetFailure` 를 1회 호출; 계획서의 `api/partner/sample/{begin,confirm}` 은 `?/begin` 액션과 `/pay/success` load 로 대체) · `/products/[code]` [샘플 구매 ₩N] → `/pay/new?product=`(`samplePayHref` · `SAMPLE_BUY_ENABLED=true`) · 🥬 사용 토글은 `/pay/new`(견적 두 벌 `quoteSample`) · 🥬 전액은 위젯 없이 `?/begin` 액션이 바로 확정 → `/pay/success?paymentId=` · `/pay/[id]` 는 seller_id 필터 404 · `/pay/fail` 은 사용자 취소면 PENDING 유지(같은 행 재시도) 그 외 code 는 FAILED · `/api/cron/reconcile` 에 `expirePartnerPayments` + stale reconcile 배선(`partner` 응답) · 캠페인 상세 `samplePaidLine` · 운영 스크립트 `partner-admin.mjs payments | refund-sample`(계획서의 `cancel-sample.mjs` 자리) · Vercel `sellery-influencer` 에 `TOSS_SECRET_KEY` · `PUBLIC_TOSS_CLIENT_KEY` · `PUBLIC_TOSS_WIDGET_VARIANT` 추가 필요(docs/deploy.md §1.2). 미구현: 웹훅 테스트 전송(e) · 강제 `CEL_INSUFFICIENT`(d) 는 운영 검증 항목으로 남김.
...
**5단계 PR-B 적용 기록(2026-09-21)**: 화면 `apps/influencer/src/routes/(console)/sales`(`getSellerSales` 1회 — 합계 4칸 · LIVE/CLEARING 카드: mini-stats · 판매 링크 복사(`SITE_URL + storeUrl`) · 내 수수료 예상(`rateLine` · 등급 보너스 · `whtLine` · 실수령 · 지급 예정일 `due_on ?? settleDue`) · 7일 막대 · 최근 주문 8건, 시뮬 토글 없음) · `(console)/settle`(폼 액션 `?/save`(`parseSettleInfoInput` → `saveSettleInfo`, 실패는 `fail(400)` 로 입력값·필드 강조 유지) · `?/setRrn`(`validateRrn` → `setSellerRrn`, **실패 응답에 입력값 없음** · 브라우저 힌트만) · `?/uploadDoc`(`uploadBizDoc`) · `GET /settle/doc`(`getBizDocSignedUrl` 300초 → 302) · 등록값은 마스킹만 표시하고 계좌번호는 저장 때마다 다시 입력 · 정산 내역 표(`listSellerSettlements` · `settlementStatusLabel`) · 계좌 미등록 보류 경고) · 매출 탭 활성(`PartnerShell TABS`) · `(demo)` 에서 `/sales` `/settle` 제거(`DEMO_PATHS`) · 홈 "정산 계좌 등록" 할 일 → `/settle` · `/my` 정산 정보 카드 → `/settle`. 개인정보 문구(수집 목적 · 암호화 · 열람 기록 · 보존 5년)는 `/settle` 원천징수 자료 카드 — `/privacy` 항목 추가는 별도. 미구현: 자동 실명확인 · 정산 실행(관리자). Vercel 서버리스 요청 본문 한도(4.5MB)가 서버의 10MB 보다 작다 — 큰 파일은 프록시 단계에서 413 이 날 수 있어 한도 문구 조정 또는 Storage 직접 업로드(서명 URL)는 후속.
...
| `/campaigns/[code]` | `js/70-campaign.js:vCampDetail` · `detActions`(seller 분기) · `scheduleModal` | `campaigns` + `campaign_events`(스레드) + 샘플 결제 정보 + `app_seller_schedule_context`(잔여 재고 · 잡힌 기간) | `receiveSample`(SAMPLE_SHIPPED → TESTING, testDue+14) · **3단계 짝(0016 · PR-B 2026-09-21)**: `?/chat`(ThreadComposer) · `?/propose`(시작일 · 기간 3/5/7 · 수량 ≤ 잔여 · 재제안 프리필) · `?/pass` · `?/accept`(저장 배송지 or ShippingFields) · `?/decline`(사유) | 3 · 3짝 |
...
1차에서 뺀 프로토타입 화면: DM(`vDM`), 랭킹(`vRank`), 셀러리 샵(`vShop`, 규제 검토 후), 추천 프로그램(`vRef`, 코드 표시만 `/my`), 판매 캘린더(`calHtml`). 일정 제안(`scheduleModal`)은 브랜드 3단계 짝 PR-B 에서 `/campaigns/[code]` 인라인 폼으로 들어갔다(2026-09-21).
...
| **4. 결제 1 — 샘플 구매** ✅ PR-A #25(0012 · 서버 계층 · 웹훅) · PR-B(화면 · reconcile · 스크립트 — 2026-09-21, §5.4 끝 "PR-B 적용 기록") | 토스 위젯 + 🥬 혼합 · 웹훅 분기 · 취소 스크립트 | 0011b(`partner_payments`·claim/confirm/fail/cancel/expire/stale·`celery_spend`·`recalc_campaign_sold_qty` 재정의) · `money.ts` 접두 상수 · `lib/partner-payment-sync.ts` · `api/partner/sample/{begin,confirm}` · `/pay/[id]`(세그먼트 2개 · 위젯 · cash=0 버튼) · `/pay/{success,fail}` · 웹훅 `slrp_` 분기 + paymentKey 3테이블 · reconcile 확장 · `scripts/cancel-sample.mjs` · 캠페인 상세 "샘플 결제 ₩N (🥬 n + 현금 m)" · 결제 화면 취소 규정 문구 | (a) 세 경로: 현금 전액 / 🥬 3 + 현금 / 🥬 전액(위젯 미렌더) 각각 `partner_payments CONFIRMED` · `campaigns SAMPLE_PURCHASED`(split 제약 통과) · `orders is_sample=true`(buyer_name·shipping 채워짐) · `campaigns.sold_qty` 는 0 유지(`recalc_campaign_sold_qty` 재정의) · 원장 −cel(cash 전액은 0행) · 홈 잔액 감소 (b) 성공 페이지 새로고침 → `already:true`, 원장 1행 유지 (c) successUrl amount 변조 → `AMOUNT_MISMATCH`, 토스 승인 기록 없음 (d) 토스 승인 직후 SQL 로 잔액을 깎아 `CEL_INSUFFICIENT` → 토스 콘솔에 전액 취소 + FAILED + 실패 문구, 원장·캠페인 없음 (e) 토스 개발자센터 "웹훅 테스트" 로 `slrp_` orderId 전송 → 200 + `payment_events` 1행; paymentKey 만 보낸 경우도 매칭 (f) `cancel-sample.mjs` → 토스 취소 + orders CANCELED + 🥬 복구, 두 번 실행해도 원장 1행 (g) 고객 결제(app-plan §11.3 #1) 회귀 없음 | `db push`(0011b) · 토스 테스트 키 그대로 · inf 호스트에서 위젯 로드 실측 1회 · 배송지 정책 확정 | **0011b** |
| **5. 매출 · 정산 자료** ✅ PR-A #27(**0013** `pgcrypto` · `sellers.rrn_enc/rrn_mask/rrn_set_at/tax_info` · `sensitive_access_log` · `settlements.sample_cel_cover` · `app_seller_sales` · `app_seller_settlements` · `app_seller_settle_info` · `app_set_settle_info` · `app_set_seller_rrn` · `app_seller_rrn_decrypt` + `@sellery/db/{partner/settle-rules, server/partner/sales, server/partner/settle}` — 2026-09-21 적용) · PR-B(화면 `/sales` `/settle` · 매출 탭 활성 · 홈/내 정보 연결 — 2026-09-21, §5.9 끝 "PR-B 적용 기록") | 실시간 매출 · 계좌/원천징수 자료 · 정산 내역 열람 | 0012→**0013**(`pgcrypto` 확장 · `sellers.rrn_enc`·`sensitive_access_log`·세금계산서 정보·`settlements.sample_cel_cover`) · `/sales` · `/settle`(폼 + 서류 업로드 + 표) · `/privacy` 항목 추가 · 홈 LIVE 카드 링크 복사(`NEXT_PUBLIC_SITE_URL + /s/<handle>/<code>`) | (a) 계좌 저장 → `sellers.bank_info` · `biz_doc_url` 이 partner-docs 경로(공개 URL 아님) · 콘솔 응답은 마스킹 (b) 주민번호 저장 후 DB 에 평문 없음, 열람 로그 1행 (c) `db query` 로 만든 `settlements` 1행이 표에 실수령(사업자면 원천징수 0)으로 표시 (d) 고객이 판매 링크로 산 주문이 `/sales` 에 반영 (e) 미등록 경고가 등록 후 사라짐 | `db push`(0013 ✅) · 정산 지급 수단 확정 · 개인정보처리방침 문구 검토 · `RRN_ENC_KEY` env(Vercel `sellery-influencer` — deploy.md §6.4) | **0013** |
| **브랜드 3단계 짝**(docs/brand-console-plan.md §6 행 3) ✅ PR-A(**0016** — 2026-09-21) · ✅ PR-B(화면 — 2026-09-21, feat/console-s3-schedule-ui · 적용 기록은 brand-console-plan §6 행 3) | `TESTING` 에서 [일정 제안](시작일 · 기간 3/5/7 · 배정 재고 ≤ 잔여) [패스] · `INVITED` 에서 [수락(배송지)] [거절] · 캠페인 스레드 답글 | 0016 `app_propose_schedule(p_seller_id, p_campaign_id, p_start, p_end, p_qty)` · `app_pass_campaign` · `app_accept_invite(…, p_shipping)` · `app_decline_invite(…, p_reason)` · `app_seller_schedule_context`(stock_left · holders · len_choices) · `app_campaign_chat('seller', …)` — `@sellery/db/partner/{schedule-rules,chat-rules}` · `server/partner/{schedule,chat}` · `SellerCampaign.action`(sellerNextAction) · 홈 할 일 testing → [일정 제안] · invited → 수락·거절 | (a) `TESTING` 상세에서 일정 제안 → `SCHEDULE_PROPOSED`(브랜드 승인 대기) · 잔여 초과 → 문구 · 플래티넘 이상이 잡은 기간 → `PERIOD_BLOCKED` 문구 (b) 브랜드 반려 뒤 다시 제안(폼 프리필 = `proposed_*`) (c) `INVITED` 수락 → `SAMPLE_APPROVED` + 배송지 · 거절 → `DECLINED` (d) 답글에 `010-…` → 경고 행 (e) 400px | — | **0016** |

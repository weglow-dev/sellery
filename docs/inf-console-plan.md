# 인플루언서 콘솔 구현 계획 — inf.sellery.co.kr (도메인 → 로그인 → 결제)
...
인플루언서 콘솔은 **새 앱이 아니라 기존 `web/`(Vercel `sellery-app`) 안의 라우트 그룹 `(partner)/influencer/**`** 로 만들고, 도메인 `inf.sellery.co.kr` 은 같은 Vercel 프로젝트에 두 번째 도메인으로 붙여 `src/proxy.ts` 가 호스트를 보고 `/influencer/*` 로 **리라이트**한다(사용자에게는 `https://inf.sellery.co.kr/home` 처럼 접두 없는 URL). 로그인은 **이메일/비밀번호(Supabase email provider, 인증 메일 필수)** 로 파트너 계정을 고객 카카오 계정과 분리하고, 가입 뒤 **신청 → 관리자 승인(SQL 함수 + 스크립트)** 으로 `sellers` 행과 연결한다. 결제 1차는 **샘플 구매(현금 토스 + 🥬 혼합)** 하나이며 토스 결제위젯·`lib/toss.ts`·`payment_events`·웹훅 라우트를 그대로 쓰되 결제 대상 테이블만 `partner_payments` 로 새로 둔다. 🥬 는 **토스 승인 뒤 확정 RPC 한 트랜잭션에서만 차감**(선차감·크론 의존 없음). 🥬 유상 충전(TOPUP)은 규제 검토가 끝나기 전에는 넣지 않는다. 세션 쿠키는 host-only 유지(고객 사이트와 콘솔 세션 자동 분리).
...
| 4 | 파트너 로그인 = 이메일/비밀번호 + 인증 메일 | `docs/app-plan.md §4.2`·0001 헤더 계약. 브랜드(법인 메일)와 같은 수단이라 승인 RPC 하나로 끝난다. 카카오는 고객 `auth.users` 와 겹쳐 `profiles.role` 단일값 충돌을 만든다. 승인 RPC 는 카카오 신원도 허용하도록 계약을 열어 둔다(§4.4) — 나중에 카카오 버튼을 추가해도 DB 는 안 바뀐다. |
| 5 | 승인은 관리자 화면 없이 SQL 함수 `approve_partner_application()` + `web/scripts/approve-partner.mjs` | 관리자 콘솔(슬라이스 4) 전에도 운영 가능. 함수는 트랜잭션 1개(sellers 연결 + profiles.role + 🥬 축하 3). |
| 6 | 게이트 = proxy 는 세션 유무만, **모든 page·서버 액션·route handler 가 `requireSeller()` 호출** | App Router 는 형제 페이지 이동 시 layout 을 재실행하지 않는다. layout 분기만으로는 승인 취소·`active=false` 계정이 세션 동안 계속 들어온다. `[mustFix]` |
...
| 10 | 세션 쿠키 host-only 유지(`cookieOptions` 미지정) | 고객 세션이 콘솔에 자동으로 붙는 혼선을 없애고 `*.vercel.app` Preview 의 부모 도메인 쿠키 불가 문제를 피한다. 나중에 `.sellery.co.kr` 공유가 필요하면 3곳(server/middleware/client)에 env 가드로 `domain` 만 추가. |
| 11 | 무상 샘플 요청 + 캠페인 목록을 결제보다 **앞 단계**에 둔다 | 결제 전까지 콘솔에서 할 수 있는 일이 계좌 등록뿐이면 실사용 검증이 결제 뒤로 밀린다. `[mustFix]` |
| 12 | 마이그레이션은 처음부터 `role`/`owner_type` 컬럼을 넣는다(0010 `partner_applications.role`, 0011 `partner_payments.owner_type`) | 브랜드 PR 이 마이그레이션을 재작성하지 않게. `[mustFix]` |

### 사용자가 결정해야 하는 것

| 항목 | 권장안 | 이유 | 결정 전 진행 |
|---|---|---|---|
| 도메인 TLD | `sellery.co.kr` 유지, `.com` 은 보유 시 리다이렉트만 | 코드·문서·약관·푸터가 전부 `sellery.co.kr`. `.com` 을 정식으로 쓰면 `company.ts`·legal·DEPLOY.md·Supabase Site URL·Redirect URLs 일괄 변경. | 가능(코드는 env `NEXT_PUBLIC_INF_HOST` 값만 바뀜) |
| apex `sellery.co.kr` 앱 이전 시점(DEPLOY.md §6) | 콘솔 1단계와 독립 — 서브도메인 먼저 | `inf.` 는 apex 가 어느 프로젝트에 있든 붙는다. 단 콘솔의 "판매 링크 복사"가 살아 있으려면 apex 이전이 선행돼야 한다. | 가능 |
| 파트너 로그인 수단 | 이메일/비밀번호(결정 4). 구글은 2차, 카카오는 미지원 | §4.1 | 가능(승인 RPC 가 두 신원 모두 허용) |
| 승인 정책 | 전원 수동 심사(스크립트) | 채널 URL·연락처를 사람이 확인하는 절차가 자가 신고 팔로워의 유일한 방어선. 자동 승인이면 신청 서버 액션이 같은 RPC 를 바로 부르면 된다. | 가능 |
...
| 시드 인플루언서 8명(`*@sellery.demo`) 처리 | 실제 계약 인플루언서 행으로 재활용(`approve … --link <seller_id>` 명시 연결), 안 쓰는 행은 `active=false` | 이메일 자동 매칭 금지(0001 헤더). | 가능 |
| 세션 쿠키 범위 | host-only(결정 10) | 브랜드 콘솔 때 재검토 | 가능 |
| 콘솔 URL 표기 | inf 호스트에서 접두 없는 경로(`/home`) | 리라이트 1겹. `/influencer/home` 그대로 두는 안은 코드는 단순하지만 URL 이 길다. | 1단계 전 |
...
```
                     ...
                     │                                                                                                                             │
 sellery.co.kr ──────┼─▶ src/proxy.ts ──(host = 고객)──▶ src/app/(customer)/{page,s,c,checkout,account,terms,privacy,login,not-found}  고객        │
                     │        │                          root layout: src/app/(customer)/layout.tsx (AppBar/Footer) — 첫 번째 root layout          │
                     │        │                                                                                                                    │
 inf.sellery.co.kr ──┼─▶      ├──(host = INF_HOST)─rewrite─▶ /influencer/*  → src/app/(partner)/influencer/**                                      │
                     │        │                          root layout: src/app/(partner)/layout.tsx (콘솔 셸) — 두 번째 root layout                 │
                     │        │                                                                                                                    │
 brand.sellery.co.kr ┼─▶      ├──(host = BRAND_HOST)─rewrite─▶ /brand/*     → src/app/(partner)/brand/**  (다음)                                   │
 ...
 demo.sellery.co.kr ──▶ Vercel project: sellery (프로토타입, 저장소 루트 정적) — 제안서 PDF 데모 주소는 별도로 유지
```

- **고객** `sellery.co.kr` — 지금의 `web/` 그대로(라우트만 `(customer)/` 로 이동, URL 불변 · apex 이전은 DEPLOY.md §6, 이 문서와 독립).
- **인플루언서** `inf.sellery.co.kr` — 이 문서.
- **브랜드** `brand.sellery.co.kr` — `hosts.ts` 표에 한 줄 + `(partner)/brand/**` 폴더 + `lib/partner/brand.ts`(§9).
- **관리자** — 별도 호스트를 두지 않고 `/admin` 경로(어느 호스트에서 열어도 같은 경로, `app_role()='admin'` + Basic Auth). 승인 화면이 생기면 §4.5 의 RPC 를 그대로 호출.
...
│  ├─ supabase/{server,client}.ts         EDIT  팩토리가 공통 옵션(cookieOptions) 을 한 곳(lib/supabase/options.ts)에서 받도록 시그니처만 정리(값은 미지정 유지)
│  ├─ auth.ts                             EDIT  isPartnerUser(user) — user_metadata.partner_role 판정(비신뢰 값 · 게이트에 쓰지 않는다는 주석, §4.2)
│  ├─ partner/seller.ts                   NEW   getSellerContext() · requireSeller() · SellerContext 타입
...
│  ├─ auth/callback/route.ts              EDIT  파트너 계정이면 ensureCustomer 생략 (§4.2)
│  ├─ auth/confirm/route.ts               NEW   token_hash 방식 인증·재설정 링크: verifyOtp({ type, token_hash }) → safeNext(next) 302, 파트너면 ensureCustomer 생략 (§4.1)
│  ├─ auth/signout/route.ts               EDIT  consoleRoleOf(host) 가 있으면 consolePath(role,'/login'), 없으면 / 로 303
...
│        ├─ (public)/{login,signup,verify-sent,apply,pending,password,password/new}/page.tsx
...
web/scripts/
├─ approve-partner.mjs                    NEW   list · approve <id> [--link <id>] · reject <id> "<사유>"
└─ dev-seller.mjs                         NEW   auth.admin.createUser(email_confirm) + approve(p_link_id) 한 번에 (production 거부)
supabase/migrations/
├─ 0010_partner_applications.sql          NEW   §5.4
...
- **파티션(소유) 갱신** — `docs/app-plan.md §10.1` 표에 추가: **I 인플루언서 콘솔** = `(partner)/influencer/**`, `lib/partner/seller.ts`, `lib/partner/sample.ts`, `lib/partner-payment-sync.ts`, `api/partner/**`, `scripts/{approve-partner,dev-seller}.mjs`. **J 브랜드 콘솔**(다음) = `(partner)/brand/**`, `lib/partner/brand.ts`. 공유 파일(`proxy.ts`, `hosts.ts`, `(customer)/` 이동, `(partner)/layout.tsx`, `partner-shell.tsx`, `money.ts`, 0010/0011) 은 1단계 PR 에서 먼저 병합하고 이후 변경은 공동 리뷰.
...
|---|---|---|---|---|
| `NEXT_PUBLIC_INF_HOST` | `inf.sellery.co.kr` | (없음) | (없음 또는 `inf.localhost:3000`) | 호스트 리라이트 표. 없으면 경로 모드. |
| `NEXT_PUBLIC_BRAND_HOST` | (브랜드 때) `brand.sellery.co.kr` | (없음) | (없음) | 같은 표의 두 번째 행 |
| `SLACK_WEBHOOK_URL` | 선택 | 선택 | 선택 | 파트너 신청 접수 알림 한 줄 — 본문은 신청 id · 활동명 · 플랫폼 만(연락처·이메일·채널 URL 은 넣지 않는다, §4.3). 없으면 무시 |
| `NEXT_PUBLIC_SITE_URL` | 변경 없음(`https://sellery.co.kr`) | | | 고객 사이트·판매 링크·고객 metadataBase 기준. 콘솔 metadataBase 는 `NEXT_PUBLIC_INF_HOST` 로 계산. |
...
| 순서 | 어디서 | 무엇을 | 확인 |
|---|---|---|---|
| 1 | Vercel `sellery`·`sellery-app` → Settings → Domains | `sellery.co.kr` 이 지금 어느 프로젝트에 있는지(또는 미연결) 기록. DNS 관리 주체(Vercel NS vs 등록기관/Cloudflare) 확인 | DEPLOY.md §1 표에 현재값 기입 |
| 2 | Vercel `sellery-app` → Domains | `inf.sellery.co.kr` 추가 (apex 위치와 무관하게 독립 추가 가능, 같은 팀이라 TXT 검증 없이 통과되는 것이 보통) | Vercel 이 CNAME 값을 표시 |
| 3 | DNS | Vercel NS 면 자동. 등록기관/Cloudflare 관리면 `CNAME inf → cname.vercel-dns.com`(Cloudflare 는 프록시 끔 = DNS only, 켜져 있으면 인증서 발급이 막힌다) | `curl -sI https://inf.sellery.co.kr/api/health` 200 + 유효 인증서 |
| 4 | Vercel `sellery-app` → Environment Variables | **Production 만** `NEXT_PUBLIC_INF_HOST=inf.sellery.co.kr` → Redeploy | `curl -s https://inf.sellery.co.kr/api/health \| jq .hosts` 가 `mode: "host"` (§2.1 `api/health` EDIT — `hosts: { inf, brand, mode }`, 비밀 없음) |
| 5 | Supabase → Authentication → URL Configuration | Redirect URLs 에 `https://inf.sellery.co.kr/**` 추가(없으면 인증·재설정 메일 링크가 Site URL 로 떨어져 `next` 를 잃음 — DEPLOY.md §3.1). Site URL 은 그대로 | 스크린샷을 DEPLOY.md §3.1 에 반영 |
...
- `consolePath(role, path)` 는 항상 **상대 경로**를 돌려준다 — 호스트 모드 `/apply`, 경로 모드 `/influencer/apply`. `next=`·`redirect()`·proxy 규칙 5 는 전부 `consolePath` 를, `<a href>`·복사용 링크만 `consoleUrl`(절대) 을 쓴다. 호스트 모드에서 `consoleUrl` 은 절대 URL 이라 `${origin}` 을 또 붙이면 안 되고, `next` 에 절대 URL 을 넣으면 `safeNext` 가 `/` 로 떨어뜨린다. `/auth/*` 는 리라이트 제외 경로라 어느 헬퍼도 거치지 않고 접두 없이 그대로 쓴다(`emailRedirectTo` 등 — §4.1).

`web/src/proxy.ts` 규칙(코드 주석과 §8 curl 검사로 고정):

1. `rewrite` 는 **내부 URL 에만**, `redirect` 는 **원 요청 pathname 에만** 적용한다(리라이트 결과가 다시 308 조건에 걸리는 루프 금지). `[mustFix]`
2. `host ∈ HOST_PREFIX` 이고 pathname 이 `REWRITE_EXCLUDE` 로 시작하지 않으면 → `rewriteTo = prefix + pathname`. pathname 이 이미 `prefix` 로 시작하면 접두를 뗀 경로로 308(정규 URL 하나).
...
5. 리라이트 대상(또는 경로 모드의 `/influencer/*`) 중 `CONSOLE_PUBLIC_PATHS` 가 아닌 경로에서 `user` 가 없으면 → `consolePath(role, "/login") + "?next=" + consolePath(role, <접두 없는 경로>)` 302 — 둘 다 상대 경로(호스트 모드 `/login?next=/home`, 경로 모드 `/influencer/login?next=/influencer/home`), `safeNext` 가 같은 오리진 경로만 통과시키므로 절대 URL 을 넣지 않는다. **DB 는 조회하지 않는다.**
...
| 가입 | `signUp({ email, password, options: { emailRedirectTo: window.location.origin + '/auth/confirm?next=' + encodeURIComponent(consolePath('seller','/apply')), data: { partner_role: 'seller', display_name } } })` — 콜백·confirm 라우트는 리라이트 제외라 콘솔 접두를 붙이지 않고, `next` 는 `consolePath`(상대 경로: 호스트 모드 `/apply`, 경로 모드 `/influencer/apply`) 로 만든다(§3.2). 인증 전엔 세션이 없으므로 `/verify-sent` 화면. |
...
| 세션 | Supabase Auth 쿠키(host-only) 그대로. `safeNext` 는 같은 오리진 경로만 통과 → inf 호스트에서 `next=/home` 은 콜백 후 `https://inf.sellery.co.kr/home` 으로 돌아와 리라이트된다. 고객 호스트 ↔ 콘솔 크로스 호스트 `next` 는 계속 거부(의도). |

### 4.2 콜백 — 파트너 계정은 `customers` 행을 만들지 않는다

`auth/callback/route.ts`: `exchangeCodeForSession` 뒤 **`user.user_metadata.partner_role` 이 있으면 `ensureCustomer` 를 건너뛴다**(`lib/auth.ts isPartnerUser(user)`). `next` 접두나 요청 host 로 판정하지 않는다 — Preview 경로 모드·비밀번호 재설정(`next=/password/new`) 에서 조건이 어긋난다. `[mustFix]` `?welcome=1` 은 고객 전용이므로 파트너면 붙이지 않는다. `handle_new_user` 트리거는 그대로 `profiles(role='customer')` 를 만든다 — 승인 전까지 customer. `auth/confirm/route.ts`(§4.1, `verifyOtp` 뒤) 도 같은 분기를 쓴다.

`partner_role` 은 사용자가 `supabase.auth.updateUser({ data })` 로 언제든 바꿀 수 있는 **비신뢰 값**이다 — `ensureCustomer` 생략·`?welcome` 생략 같은 무해한 분기에만 쓰고 권한·게이트·집계에는 절대 쓰지 않는다(`lib/auth.ts isPartnerUser` 주석에 명시). 게이트의 진실은 `sellers.user_id and active`(§4.4).

### 4.3 가입 → 심사 → 연결 → 진입 시퀀스

```mermaid
sequenceDiagram
  participant B as 브라우저(inf.sellery.co.kr)
  participant P as proxy.ts
  participant S as Next 서버(서버 액션 · service role)
  participant A as Supabase Auth
  participant DB as Postgres
  participant OP as 운영자(스크립트)
  B->>A: signUp(email, pw, data.partner_role='seller')  → 인증 메일
  B->>B: /verify-sent
  A-->>B: 메일 링크 → /auth/confirm?next=/apply&token_hash=…&type=signup (다른 기기에서 열어도 됨)
  B->>P: GET /auth/confirm (리라이트 제외)
  P->>S: verifyOtp(type, token_hash) → 세션 → partner_role 있음 → ensureCustomer 생략 → 302 /apply
  B->>P: GET /apply (host=inf → rewrite /influencer/apply, 세션 있음)
  S->>DB: requireSeller() → sellers.user_id=uid 없음 → partner_applications 최신 1건 없음 → state 'guest'
  S-->>B: 신청 폼(활동명·플랫폼·핸들·채널 URL·팔로워·연락처·추천 코드·약관)
  B->>S: submitApplication(form)  [rate limit · rejectCrossSite]
  S->>DB: partner_applications insert(role='seller', user_id=uid, status pending)  → Slack 알림(선택 · 신청 id · 활동명 · 플랫폼 만)
  S-->>B: /pending ("심사 중 · 보통 1~2 영업일")
  OP->>DB: node web/scripts/approve-partner.mjs list → 채널 URL·연락처 사람이 확인
  OP->>DB: approve <application_id> [--link <seller_id>]  → approve_partner_application() 1 트랜잭션
  DB->>DB: email_confirmed_at(또는 identities kakao) 검사 → sellers insert 또는 user_id 연결 → seller_channels 1행 → profiles.role='seller' → celery_ledger +3 signup_bonus → 신청 approved (--link 연결 경로는 채널·축하 🥬 생략)
  B->>P: GET /home (새로고침)
  S->>DB: requireSeller() → sellers(active) 있음 → SellerContext
  S-->>B: 홈(지금 할 일 3장 · 내 링크 · 오늘 매출)
```

반려: `reject_partner_application(id, note)` → `/pending` 이 `admin_note` + [다시 신청](새 pending 1건, 부분 유니크로 동시 2건 방지).

Slack 알림 본문은 `신청 id · 활동명 · 플랫폼` 만(연락처·이메일·채널 URL 은 넣지 않는다 — 상세는 `approve-partner.mjs list`). 개인정보가 제3자(Slack)로 나가지 않으므로 `/privacy` 의 수집·이용·제3자 제공 항목에는 넣지 않는다.
...
| requireSeller() | `createAdminClient()` 로 `sellers.user_id = user.id and active` 1행 → 없으면 `partner_applications` 최신 1건으로 `guest / pending / rejected / suspended` 분기 → 각각 `/apply`, `/pending`, `/pending?rejected=1`, `/pending?suspended=1` 로 `redirect(consolePath('seller', …))`. **suspended**: 최신 신청이 `approved` 인데 `sellers.active=false`(또는 `user_id` 연결이 풀림)이면 `/pending?suspended=1`("이용이 정지되었습니다 · 고객센터 이메일") 로 보내고 재신청 폼은 열지 않는다 — 열면 새 pending 이 생겨 운영자 큐를 오염한다(부분 유니크는 pending 중복만 막는다). **콘솔 게이트의 진실은 `sellers.user_id and active`** 이고 `profiles.role`/`app_role()` 은 관리자·집계 판정용(코드 주석으로 고정). `[mustFix]` | `lib/partner/seller.ts` — **모든 page·서버 액션·`api/partner/*` 가 직접 호출**(layout 에만 두지 않는다) |
| 서버 액션·API | `rejectCrossSite`(`sec-fetch-site` + JSON) → `requireSeller()` 로 `seller.id` 를 세션에서 얻는다. 클라이언트가 보낸 `seller_id`·금액은 믿지 않는다. 신청 액션·결제 begin 은 checkout 과 같은 rate limit(30분 20건). `[mustFix]` | `**/actions.ts`, `api/partner/**` |
...
`approve_partner_application(p_application_id uuid, p_link_id uuid default null, p_admin_note text default null) returns jsonb` — security definer, `service_role` 만 execute(public/anon/authenticated 회수). `p_link_id` 는 신청 `role` 에 따라 `sellers.id`(seller) 또는 `brands.id`(brand) — 브랜드 때 시그니처를 드롭·재생성하지 않기 위한 중립 이름(§9). 한 트랜잭션:

1. 신청 `for update`, `status='pending'` 아니면 `{ok:false, code:'NOT_PENDING'}`.
2. **신원 확인 — 계약 개정** `[mustFix]`: `auth.users.email_confirmed_at is not null` **또는** `exists (select 1 from auth.identities where user_id=… and provider='kakao')`. 0001 헤더의 조건을 이렇게 명시적으로 넓혀 `docs/data-model.md §7` 에 기록한다(지금은 이메일 계정만 오지만 카카오 버튼을 나중에 켜도 DB 는 안 바뀐다).
3. `p_link_id` 가 있으면 그 `sellers` 행의 `user_id` 가 null 인지 확인 후 연결(시드 인플루언서·기존 오프라인 계약자용. **이메일 자동 매칭은 하지 않는다**). 없으면 `sellers` insert(code `'s'||nextval`, name·handle·platform·followers·email(신청 이메일)·grade 스타터·active·`contact_phone`(NEW)·`ref_code` 발급 `SLRY-XXXX`). `handle` 은 DB 규약대로 `'@' || normalize(신청 핸들)`(소문자·앞 `@` 제거 후 다시 `@` 1개 부착) 로 저장한다 — `sellers.handle` 은 `@` 포함이 규약(0001 컬럼 주석 · `lib/campaign.ts` L50), URL 은 `normalizeHandle()` 이 뗀다. 유니크 충돌 시 접미 `-2`.
   **`p_link_id` 연결 경로에서는 4단계(채널)와 6단계(축하 🥬)를 건너뛴다** — 기존 행에 primary 채널이 있으면 그대로 두고(없을 때만 insert; `seller_channels` 의 부분 유니크 `(seller_id) where is_primary`(0001) 에 걸려 RPC 전체가 실패하지 않도록), `celery_ledger` 에 같은 seller 의 `signup_bonus` 행이 있으면 지급하지 않는다(`not exists` 가드 — 시드 원장에 이미 있음, seed.sql). `contact_phone`·`email` 이 비어 있을 때만 신청값으로 채운다.
4. `seller_channels` 1행(platform·handle·url·followers·is_primary=true·verified=false·vcode null).
5. `profiles.role='seller'`.
6. `celery_ledger(+platform_settings.signup_bonus_cel, reason 'signup_bonus', owner seller)`.
7. 신청 `referral_code` 가 `sellers.ref_code` 와 일치하면 `referred_by` 세팅.
8. 신청 `status='approved', seller_id, decided_at, admin_note`.

`reject_partner_application(p_application_id, p_admin_note)` — pending → rejected. 두 함수 모두 `role='brand'` 신청이면 `brands` 를 연결하도록 분기만 남겨 둔다(브랜드 PR 이 채운다).
...
### 4.7 관리자 화면 없이 승인하는 최소 구현

- `node web/scripts/approve-partner.mjs list` — pending 신청을 표로(활동명 · 이메일 · 플랫폼 · **채널 URL** · 팔로워 · 연락처 · 추천 코드 · 신청일). 승인자는 채널 URL 을 열어 사람이 확인한다(심사 체크리스트: 채널 실존 · 핸들 일치 · 건강·웰니스 카테고리 · 연락처 응답). `[mustFix]`
- `approve <application_id> [--link <id>]`(`--link` 는 신청 `role` 에 따라 `sellers.id`/`brands.id` → `p_link_id`), `reject <application_id> "<사유>"` — service role 로 위 RPC 호출. production 허용(dev-user.mjs 와 달리). 대안: `npx supabase db query --linked "select approve_partner_application('…')"`.
- `node web/scripts/dev-seller.mjs --email … --seller s1` — `auth.admin.createUser(email_confirm:true, user_metadata.partner_role='seller')` + `partner_applications` 1행 + approve(`p_link_id` = 그 seller) 를 한 번에(production 거부 가드). 연결 경로라 채널·축하 🥬 는 건너뛴다(§4.5 3단계 — 시드 행에는 둘 다 이미 있다). 시드 인플루언서 8명(`*@sellery.demo`)은 실제 메일을 못 받으므로 이 스크립트로만 연결한다.
- 승인 알림 메일은 1차에 없음(신청자가 `/pending` 새로고침). 관리자 콘솔(슬라이스 4)이 오면 같은 RPC 를 서버 액션에서 호출.

### 4.8 사용자가 대시보드에서 할 일 (3단계 착수 전)

Supabase → Authentication → Providers → **Email ON · Confirm email ON · 최소 비밀번호 8** / URL Configuration → Redirect URLs 에 inf 호스트(§3.1) / **Custom SMTP**(Resend·SES 등; 발신 `no-reply@sellery.co.kr`, SPF/DKIM DNS 레코드 — 기본 SMTP 는 시간당 소량·팀원 주소 위주라 외부 인플루언서에게 안 간다) / 이메일 템플릿(Confirm signup · Reset password) 한국어화 — 링크는 `{{ .ConfirmationURL }}` 이 아니라 `{{ .RedirectTo }}&token_hash={{ .TokenHash }}&type=signup`(Reset password 는 `type=recovery`) 로 바꾼다(§4.1 token_hash 방식 — `.RedirectTo` 에 inf 호스트와 `next` 가 실린다). 문구는 역할 중립('셀러리 파트너')으로 — 템플릿은 프로젝트당 1벌이라 브랜드 가입 메일과 공유된다. **현재 대시보드 값(Email provider 상태·Confirm 여부·SMTP·Redirect URLs)을 `web/DEPLOY.md §3.5` 에 기록**한다 — 저장소로는 확인 불가. `[mustFix]` 로컬 `supabase/config.toml` 은 클라우드에 영향 없으니 손대지 않는다.
...
- [ ] `partner_applications`·`partner_payments`·`celery_ledger`·`settlements`·`payouts` 는 RLS on + revoke all(서버 전용). 0007 default privileges 로 service role 만 접근.
...
- [ ] 채널 인증 코드 `SLRY-XXXX` 는 저장만, `verified=true` 전환은 운영자(스크립트)만.
- [ ] 콘솔 페이지 `noindex` + `robots.ts` disallow.
- [ ] 로그아웃 `POST /auth/signout` 재사용 — `auth/signout/route.ts` 를 고쳐(§2.1 EDIT) `consoleRoleOf(host)` 가 있으면 `consolePath(role,'/login')`, 없으면 `/` 로 303. 지금은 `new URL("/", request.url)` 고정이라 콘솔에서 로그아웃하면 `/` → `/influencer` 리라이트 → 세션 없음 → `/login` 으로 두 번 튄다.
...
**0010_partner_applications.sql**

| 컬럼 | 타입 · 제약 |
|---|---|
| `id` | uuid pk default gen_random_uuid() |
| `user_id` | uuid not null → auth.users on delete cascade |
| `role` | text not null check in ('seller','brand') `[mustFix 처음부터]` |
| `email` | text not null (가입 이메일 스냅샷) |
| `name` · `handle` · `platform` · `channel_url` | text (platform check in ('instagram','youtube','naver','tiktok')) |
| `followers` | integer check ≥ 0 |
| `category` | text (CATMAP 그룹) |
| `phone` | text |
| `referral_code` | text (프로토타입에서 버려지던 값 보존) |
| `biz_no` | text (브랜드용, nullable) |
| `note` · `admin_note` | text |
| `status` | text not null default 'pending' check in ('pending','approved','rejected') |
| `seller_id` | uuid → sellers on delete set null (승인 시 기록) |
| `decided_at` · `created_at` · `updated_at` | timestamptz |
| 인덱스 | `partner_applications_pending_uidx` unique (user_id, role) where status='pending' · (status, created_at) |
| RLS | on + revoke all (서버 전용) |

같은 파일: `sellers.contact_phone text`(service role 전용, 승인 시 신청 phone 복사) · `sellers.sample_address jsonb`(샘플 배송지 기본값) · 함수 `approve_partner_application` · `reject_partner_application`(§4.5).
...
|---|---|---|---|---|
| `/login`(콘솔 호스트의 `/login` 은 여기로 리라이트 — 고객 카카오 `/login` 아님, §3.2) `/signup` `/verify-sent` `/password` `/password/new` | `login.html` (ACCOUNTS·fJoin·비번 재설정) | — | Supabase Auth (`signInWithPassword`, `signUp`, `resetPasswordForEmail`, `updateUser`) | 2 |
| `/apply` `/pending`(`?rejected=1` · `?suspended=1`) | (없음 — glo `seller/_lib.ts` 게이트 + 신청 폼) | `partner_applications` 최신 1건 · `sellers.active`(suspended 판정, §4.4) | `submitApplication` → insert(role 'seller') · Slack 알림(신청 id·활동명·플랫폼만). suspended 면 신청 폼 없음 | 2 |
| `/home` | `js/20-seller.js:vSellerHome` 중 **지금 할 일 · 진행 중(LIVE) 카드 · 내 자산** 3개 위젯 | `campaigns`(본인, 상태별) · `orders` 집계(오늘 매출) · `celery_balances` · `grade_tiers` · `sellers.bank_info` 유무 | — (승인 직후 "지금 할 일" = 채널 인증 · 계좌 등록 · 첫 상품 둘러보기) | 3 |
...
| `/my` | `vMy`(프로필 · 채널 · 추천 코드) · `channelModal` · `verifyModal` | `sellers` · `seller_channels` · `ref_code` | `saveChannel`(핸들/플랫폼 변경 시 verified=false) · `issueVerifyCode`(`SLRY-XXXX` 저장만) · `saveSampleAddress` | 3 |
...
|---|---|---|---|---|---|
| **1. 호스트 분리 골격** | inf.sellery.co.kr 이 앱을 가리키고 콘솔 셸이 뜬다 | `src/app/(customer)/` 이동(root layout + 고객 라우트, URL 불변) · `lib/hosts.ts`(`consoleUrl`/`consolePath`) · `proxy.ts` 리라이트/308(`*.vercel.app` 제외)/세션 게이트 · `supabase/middleware.ts` `{response,user}` + `rewriteTo` · `lib/supabase/options.ts`(쿠키 옵션 1곳) · `(partner)/layout.tsx`(두 번째 root layout) + `partner-shell.tsx` · `(partner)/influencer/page.tsx`(→ `/home`) · `(partner)/influencer/home/page.tsx`("준비 중" 카드) · `auth/signout` 303 목적지 · `api/health` `hosts` · `robots.ts`(콘솔 호스트 전체 disallow) · `next.config.ts allowedDevOrigins` · `.github/CODEOWNERS` · app-plan §10.1 파티션 I·J · DEPLOY.md §1·§2 | (a) `https://inf.sellery.co.kr/` → 콘솔 셸(로그인 안내 — `/` → `/influencer` 리라이트가 `(partner)/influencer/page.tsx` 에 착지), `/influencer` 는 `/` 로 308, `https://sellery-app.vercel.app/influencer/`(apex 이전 전후 모두) 와 PR Preview `/influencer/` 도 같은 화면, `curl -s https://inf.sellery.co.kr/robots.txt` 가 `Disallow: /`, `curl -s https://inf.sellery.co.kr/api/health \| jq .hosts` 가 `mode: "host"` (b) **`curl -D - https://inf.sellery.co.kr/home` 응답에 `set-cookie: sb-…` 회전 쿠키가 실린다**(세션 있는 쿠키로 요청) (c) **`curl -I https://inf.sellery.co.kr/auth/callback` 이 404 가 아니다**(리라이트 제외) (d) 고객 홈·`/s/…`·체크아웃·`not-found` 회귀 없음(`(customer)/` 이동 후 URL 동일 · app-plan §11.2 시나리오) (e) 308 루프 없음(`curl -IL` 홉 ≤ 2) `[mustFix]` | Vercel Domains 추가 · DNS CNAME · Production env `NEXT_PUBLIC_INF_HOST` · Supabase Redirect URL(§3.1 1~5) · TLD 확정 · `main` 보호 규칙에서 Require review from Code Owners 를 켤지 결정(켜면 required approvals 0 → 1 로 바뀌어 팀 2명 규칙이 달라진다 — 켜지 않으면 CODEOWNERS 는 리뷰어 자동 지정용일 뿐) | 없음 |
| **2. 로그인 · 가입 · 신청 · 승인** | 이메일 계정으로 콘솔에 들어온다 | 0010 · `(public)/{login,signup,verify-sent,apply,pending,password,password/new}` · `lib/partner/seller.ts` `requireSeller()`(guest/pending/rejected/suspended) · `auth/callback` 파트너 분기 · `auth/confirm/route.ts`(token_hash 검증, §4.1) · `submitApplication`(rate limit·Slack) · `scripts/approve-partner.mjs`(list/approve/reject) · `scripts/dev-seller.mjs` · `npm run gen:types` · data-model §7·§9 · DEPLOY.md §3.5 | (a) **팀원이 아닌 실제 이메일**로 `/signup` → 인증 메일 수신 → 링크를 **가입한 브라우저가 아닌 기기(휴대폰 메일 앱)** 에서 열어도 `/apply` 도달(token_hash) → 제출 → `/pending` (b) `approve-partner.mjs list` 에 채널 URL 표시 → `approve <id>` → 새로고침 시 `/home` 에 활동명·스타터·🥬 3 (c) `db query` 로 `sellers.user_id`·`profiles.role='seller'`·`celery_ledger +3`·`seller_channels` 1행이 한 트랜잭션에 생김, 미인증 계정은 `approve` 거부 (d) 승인 취소(`active=false`) 후 `/products` 직접 접근 → `/pending?suspended=1`, 신청 폼 없음(게이트가 page 에 있음) (e) `/password` 메일 → 다른 기기에서 링크를 열어도 `/password/new` → 새 비번 로그인 (f) `sellery.co.kr` 카카오 세션 브라우저에서 inf 는 미로그인(쿠키 분리) (g) 파트너 계정에 `customers` 행 없음 (h) `dev-seller.mjs --seller s1` 로 시드 지유 연결 → 같은 화면 · 채널 1행 유지 · 원장 signup_bonus 1행 유지 | Supabase Email provider ON · Confirm ON · 최소 8 · **Custom SMTP + SPF/DKIM** · 템플릿 한국어(token_hash 링크 · 역할 중립 문구) · 대시보드 값 DEPLOY.md §3.5 기록 · `npx supabase db push --linked` 승인 · 승인 정책 확정 | **0010** |
| **3. 돈 없이 첫 캠페인** | 상품 갤러리 · 무상 샘플 요청 · 캠페인 목록 · 홈 위젯 · 내 정보 | 0011 중 `app_sample_quote`·`app_request_free_sample`·`campaigns.sample_shipping`·`sellers.sample_address` 를 **0011a** 로 먼저 적용(결제 테이블은 4단계) · `/home` 3위젯 · `/products` · `/products/[code]`(버튼 분기, 샘플 구매는 "4단계 예고" 비활성) · `/campaigns` · `/campaigns/[code]`(스테퍼·스레드·수령 확인) · `/my`(채널·인증 코드 저장·배송지) | (a) 승인 계정이 브론즈 조건 상품에서 [무상 샘플 요청] → 배송지 입력 → `campaigns SAMPLE_REQUESTED` + `sample_shipping` (b) 같은 상품 재요청 → `ALREADY_ACTIVE`, 월 한도 소진 시 버튼이 [샘플 구매 ₩N] 으로 바뀜(quote 결과) (c) `db query` 로 상태를 SAMPLE_SHIPPED 로 바꾸면 홈 "지금 할 일" 카드 변경 → [수령 확인] → TESTING + testDue (d) 다른 인플루언서 캠페인은 URL 을 바꿔도 404 (e) 400px 폭 가로 스크롤 없음 | `db push`(0011a) · 등급 보너스 반영 여부 확정 | **0011a** |
...
### 8.2 호스트 · 세션 (1단계, curl)
```
curl -sI https://inf.sellery.co.kr/                      # 200, x-robots-tag 또는 meta noindex
curl -sI https://inf.sellery.co.kr/influencer/home       # 308 → /home (홉 1)
curl -sIL https://inf.sellery.co.kr/home | grep -c '^HTTP'   # ≤ 2 (루프 없음)
curl -sI https://inf.sellery.co.kr/auth/callback         # 404 아님(리라이트 제외)
curl -sI https://sellery.co.kr/influencer/home           # 308 → https://inf.sellery.co.kr/home (apex 이전 후)
curl -sD - -H "cookie: sb-…=<세션>" https://inf.sellery.co.kr/home | grep -i set-cookie   # 회전 쿠키 실림
curl -sI https://sellery-app.vercel.app/influencer/home  # 200 (경로 모드, 308 없음 — apex 이전 전후 모두, *.vercel.app 은 규칙 3 제외)
curl -s  https://inf.sellery.co.kr/robots.txt             # Disallow: / (리라이트 제외 · 콘솔 호스트 전체 비색인)
curl -s  https://inf.sellery.co.kr/api/health | jq .hosts  # mode: "host"
curl -sI https://inf.sellery.co.kr/login                  # 200 콘솔 로그인(/influencer/login 으로 리라이트) — 고객 카카오 로그인이 아님
```

### 8.3 브라우저 시나리오 (2~3단계)
1. 실제 외부 이메일 가입 → 메일(링크는 다른 기기에서 연다 — token_hash) → `/apply` → `/pending` → 스크립트 승인 → `/home`.
2. 승인 취소(`active=false`) 후 형제 페이지 이동(`/products` → `/campaigns`)에서도 `/pending?suspended=1` 로(신청 폼 없음) — layout 이 아니라 page 게이트 확인.
3. 두 인플루언서 계정으로 서로의 `/campaigns/[code]`·`/pay/[id]` 404.
4. 고객 사이트 카카오 세션 브라우저에서 콘솔 미로그인.
5. 모바일 폭(375px) 하단 탭·폼·결제 세그먼트.
...
| `partner_applications.role` + `approve_partner_application(p_application_id, p_link_id, p_admin_note)` + `approve-partner.mjs --link` | `role='brand'` 분기에서 `p_link_id` 를 `brands.id` 로 읽어 `brands.user_id` 연결 — 인자 이름이 중립이라 **시그니처 변경(드롭·재생성·스크립트 수정) 없음**. 신청 폼에 사업자번호(`biz_no`) |
...
| `scripts/dev-seller.mjs` 구조 | `dev-brand.mjs` |
| 관리자(슬라이스 4) | `/admin` 경로(호스트 없음) + `app_role()='admin'` + Basic Auth 이중 잠금. 승인·반려·샘플 취소·payouts.paid 화면은 이 문서의 RPC/스크립트를 버튼으로 감싼다. `partner_payments CANCEL_PENDING`·`payment_events handled=false` 큐 화면. |
| 세션 쿠키 host-only · `.sellery.co.kr` 공유 전환 | 브랜드 콘솔 때 재검토 — 전환 시 `lib/supabase/options.ts` 한 곳 + Preview 분기 + `clearAuthCookiesAtScopes` 실측 |
| 문서 | `docs/app-plan.md §12` 슬라이스 2 행에 "호스트 brand.sellery.co.kr · (partner)/brand" 반영, §10.1 파티션 J |

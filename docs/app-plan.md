...
| `customers` 비회원 행(`guest=true` — 이름·연락처·이메일 · 조회 토큰 해시, 0021) | 주문이 있으면 주문 거래기록과 동일 · 주문이 없으면 30일 | `purge_checkout_pii()` 가 주문 없는 비회원 행을 **삭제**(`checkout_sessions.customer_id` set null). 주문 있는 행은 주문과 함께. |
...
| `/checkout` | 서버 → `CheckoutClient` | `?c={code}&o={optIdx}&q={qty}[&g=1]` | user `getUser()` — **미로그인이면 리다이렉트하지 않고 선택 카드**(비회원 구매 2026-09-22): [카카오로 로그인하고 구매](`/login?next=`) · [비회원으로 구매](`&g=1`). `g=1` 이면 비회원 폼(주문자 이름·연락처·이메일(선택)·동의 `/privacy#guest` + 배송지 · customerKey 토스 ANONYMOUS · "기본 배송지 저장" 없음). anon `campaign_card`, user `customers`(기본 배송지 프리필 — 회원만) | 파라미터 결측/범위 밖 → 판매 페이지로 redirect; `isBuyable(card, qty)` 실패(LIVE 아님·`today` 범위 밖·재고 부족) → `.notice`(danger) + 돌아가기, 위젯 렌더 안 함 |
...
| `/account/orders/[code]` | 서버 | `params.code` | service `fetchMyOrder(user.id, code)` — user_id 불일치면 `notFound()`. 본문은 `apps/shop/src/lib/components/OrderDetail.svelte`(비회원 상세와 공용) | 미로그인 → `/login?next=` |
| `/orders/lookup` (비회원 구매 2026-09-22 · 0021) | 서버 + form action `?/lookup` | 폼 주문번호 + 연락처 · `?code=` 프리필 · `?msg=expired` | `rateLimit` **IP 30분 5회** → `parseGuestLookupInput` → service `lookupGuestOrder`(0021 `app_guest_order_lookup` — 비회원 주문(`customers.guest`)만 · 연락처는 `customers.phone_norm`/`orders.buyer_phone`/`shipping.phone` 중 하나) → 새 조회 토큰을 HttpOnly 쿠키 `slry_guest_<code>`(90일 · Lax · prod Secure)로만 → 303 `/orders/g/<code>` | 주문번호 없음·연락처 불일치·회원 주문을 구분하지 않는 한 문구(`GUEST_LOOKUP_FAIL`) · robots noindex + `robots.txt` Disallow `/orders` · `cache-control: private, no-store`(orders/+layout.server.ts) |
| `/orders/g/[code]` (비회원 주문 상세) | 서버 | `params.code`, 쿠키 `slry_guest_<code>` | service `fetchGuestOrder(code, token)` = `app_guest_order_verify`(sha256 대조) → `fetchOrderById`(회원 상세와 같은 조인 · `MyOrder`) → `OrderDetail`(← 주문 조회 · 비회원 안내 띠 · 환불 `RefundButton` · 문의 `/cs/new?campaign=&order=`) | 쿠키 없음·불일치·주문 없음 → 303 `/orders/lookup?code=&msg=expired`(404 로 구분하지 않는다) |
...
**비회원 구매 2026-09-22 (0021)** — 위 §7.1~§7.4 는 비회원에게도 같다. 다른 점은 인증뿐: `/api/checkout` 이 `guest` 본문으로 `customers` 비회원 행(`guest=true` · 체크아웃당 1행)을 만들고 세션 `user_id=null` + HttpOnly 쿠키 `slry_gck`(=세션 id · 45분)를 심는다 → `/api/payments/confirm` 은 `getUser()` 대신 이 쿠키로 세션 소유를 확인하고, 확정 뒤 `app_guest_token_issue` 로 조회 토큰(32바이트 난수 · DB 에는 sha256 해시만)을 `slry_guest_<주문번호>`(90일)로 심는다 → `/orders/g/<code>`(상세) · `/api/payments/cancel`(환불) · `/cs/new`(문의 — 0018 `client_token` 그대로)가 그 쿠키로 통과한다. 쿠키가 없으면 `/orders/lookup`(주문번호 + 연락처 · IP 30분 5회)이 `app_guest_order_lookup` 으로 토큰을 재발급(회전)한다. `purge_checkout_pii` 는 주문 없는 비회원 행을 30일 뒤 삭제한다(§5.1). 웹훅·reconcile 은 `user_id` 를 보지 않으므로 변경 없음. 관리자 주문 목록(0020 `app_admin_orders`)은 `orders.buyer_name` 을 쓰므로 비회원 주문도 그대로 보인다.

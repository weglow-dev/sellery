...
├── scripts/check.mjs   ← CI 검사: 참조 무결성 + JS 문법
...
3. **Commit changes…** → 메시지(`mkt: 버닝온 상품 설명 문구 수정`) → **"Create a new branch for this commit and start a pull request"** → 브랜치명 `copy/burningon-desc`
...
4. 로컬에서 `node scripts/check.mjs`로 CI와 같은 검사를 돌린 뒤 PR. 브랜치 `feat/…`, 커밋 `feat: …`.
...
- **CI `ci` 통과 필수** — `scripts/check.mjs`가 (1) `index.html`이 가리키는 css/js/assets 파일이 실제로 있는지(참조 무결성), (2) js 파일 문법을 검사합니다.

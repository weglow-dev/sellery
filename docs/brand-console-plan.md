...
> **상태(2026-09-21): 1단계 구현(골격 + 가입/로그인 · 0014 적용 · §6 행 1 "적용 기록").** 2단계부터는 계획. 인플루언서 콘솔 1~5단계(`docs/inf-console-plan.md §7`, PR #19~#30)가 끝났고, 그 골격(`(console)` 그룹 · `PartnerShell` · `requireSeller()` · form action · `{ok, code}` RPC 관례 · `partner-admin.mjs`)을 **그대로 복사**해 `apps/brand` 를 localStorage 데모에서 브랜드 콘솔로 바꾼다. 인플루언서 계획서 §9 "이후(브랜드 콘솔·관리자)에 그대로 적용되는 것" 이 이 문서의 씨앗이다.
>
...
| 3단계 초대 게이트 범위 | **플래티넘 이상 전체 `PRIORITY_INVITE_GATED`**(`grade_tiers.is_priority`) — 6단계에서 `invite_cost_cel > 0`(다이아·블랙) 🥬 차감으로 교체 | 데모 `confirmInvite` 는 다이아·블랙만 🥬 10, 플래티넘은 무료. 3단계에 플래티넘을 열려면 0016 의 분기를 `invite_cost_cel > 0` 으로 바꾸면 된다(한 줄). 우선권 등급을 한 묶음으로 두는 편이 인플루언서 갤러리(6단계) 설계와 맞아 보류. |

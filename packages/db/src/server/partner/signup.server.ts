 ...
 * web/src/lib/partner/signup.ts 의 이식 — admin 클라이언트만 주입 가능하게 바뀌었다.
 * 호출자: `auth/confirm`(token_hash 인증 뒤) · `auth/callback`(PKCE 뒤, 파트너면) · `/apply`(페이지 로드 재시도 + form action).
 *
...
import { parseSignupMeta, type SignupFailCode, type SignupMetaField, isSignupFailCode } from "../../partner/signup-rules";
import { createAdminClient, type Admin } from "../admin.server";
import { notifySlack } from "./slack.server";
...
export function parseSignupRpcResult(json: unknown): SignupResult {
  const o = obj(json);
 ...
 */
export async function createSellerFromSignup(user: User, form?: unknown, admin: Admin = createAdminClient()): Promise<SignupResult> {
  ...
  const result = parseSignupRpcResult(data as Json);

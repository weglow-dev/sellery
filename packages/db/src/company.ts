...
export const COMPANY = {
  ...
  /** 이용약관 페이지 (apps/shop /terms · 본문 packages/db/src/legal/terms.ts) — 비우면 푸터가 링크 대신 텍스트로 렌더 */
  termsUrl: "/terms",
  /** 개인정보처리방침 페이지 (apps/shop /privacy · 본문 packages/db/src/legal/privacy.ts) */
  privacyUrl: "/privacy",
} as const;
...
export function warnIfCompanyPending(): void {
  ...
  console.warn(
    `[company] 사업자 정보 미확정 필드: ${pending.join(", ")} — packages/db/src/company.ts 를 확정값으로 교체하세요 (docs/app-plan.md §13).`,
  );
}

...
//
//   node --env-file=.env.local packages/db/scripts/dev-user.mjs <email> <password>   (저장소 루트에서)
//
// - 루트 .env.local 의 PUBLIC_SUPABASE_URL · SUPABASE_SERVICE_ROLE_KEY 를 쓴다 (값을 출력하지 않는다).
// - 이미 같은 이메일이 있으면 만들지 않고 skip 한다.
// - email_confirm: true 라 확인 메일 없이 바로 /login 의 개발용 폼(PUBLIC_DEV_LOGIN=1)에서 로그인할 수 있다.
...
if (!email || !password) {
  console.error("usage: node --env-file=.env.local packages/db/scripts/dev-user.mjs <email> <password>");
  process.exit(2);
}
...
if (!url || !serviceKey) {
  console.error(
    "[dev-user] PUBLIC_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY 가 없습니다 — `node --env-file=.env.local …` 로 실행하세요.",
  );
  process.exit(1);
}

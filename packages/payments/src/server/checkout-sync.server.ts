/**
 * 결제 세션 ↔ 토스 상태 동기화 — /api/payments/{confirm,webhook} · /api/cron/reconcile 의 공용 서버 로직. web/src/lib/checkout-sync.ts 의 이식 (docs/monorepo-migration.md §3.2).
 * 변경점: `NextResponse` → 표준 `Response`(`apiError` · `rejectCrossSite` 는 `Response.json`) · admin 클라이언트는 인자로 · 토스 키는 `configurePayments()` 주입.
...
export function apiError(status: number, code: string, message?: string | null): Response {
  return Response.json({ ok: false, code, message: failMessage(code, message) }, { status });
}
 ...
 * 통과하면 null, 아니면 403 BAD_ORIGIN 응답(`+server.ts` 가 그대로 return 한다).
 */

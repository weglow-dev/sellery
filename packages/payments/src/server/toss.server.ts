/**
 * 토스페이먼츠 서버 API 래퍼 — web/src/lib/toss.ts 의 이식 (계약 docs/app-plan.md §10.0 · §7 · docs/monorepo-migration.md §3.2).
 *
 * 인증: Basic base64(`${secretKey}:`). 키는 `process.env` 가 아니라 **주입값**이다 — `configurePayments({ secretKey })`(hooks) 로 넣은 기본 인스턴스를
 * `tossConfirm` · `tossCancel` · `tossGetPayment` 가 쓰고, `createToss({ secretKey })` 로 키를 명시한 인스턴스도 만들 수 있다(스크립트·테스트).
 ...
 *
 * 이 모듈은 DB 를 모른다 — 세션/주문 동기화는 checkout-sync.server.ts.
 */
import { paymentsConfig } from "./config.server";
...
export type TossApi = {
  ...
  tossConfirm(input: { paymentKey: string; orderId: string; amount: number }): Promise<TossResult>;
  /**
   ...
   */
  tossCancel(paymentKey: string, reason: string, opts: { idempotencyKey: string; cancelAmount?: number }): Promise<TossResult>;
  /**
   * GET /v1/payments/{paymentKey} — 웹훅·재조회·reconcile 의 유일한 진실.
   * 4xx(NOT_FOUND_PAYMENT_KEY 등) = 토스에 승인 기록 없음. 재조회는 기본 8초 타임아웃.
   */
  tossGetPayment(paymentKey: string, opts?: { timeoutMs?: number }): Promise<TossResult>;
};

export type CreateTossOptions = {
  /** 시크릿 키. 생략하면 호출 시점에 `configurePayments()` 값을 쓴다 */
  secretKey?: string;
  /** 테스트용 fetch 주입 */
  fetch?: typeof fetch;
  baseUrl?: string;
...
/**
 * 토스 API 인스턴스. `secretKey` 를 주면 그 키를, 아니면 매 호출마다 `paymentsConfig().secretKey` 를 읽는다(hooks 가 나중에 configure 해도 반영).
 * 예외·타임아웃·파싱 실패를 전부 TossResult 로 흡수한다 — 라우트가 try/catch 없이 status 로 분기.
 */
export function createToss(opts: CreateTossOptions = {}): TossApi {
  const fetchFn = opts.fetch ?? fetch;
  const base = opts.baseUrl ?? TOSS_BASE;

  function authHeader(): string {
    const secret = opts.secretKey || paymentsConfig().secretKey;
    if (!secret) throw new Error("TOSS_SECRET_KEY is not set — configurePayments({ secretKey }) in hooks.server.ts from $env/dynamic/private.");
    // Basic base64(`${secret}:`) — 키는 ASCII 라 btoa 로 충분 (Node 22 · 브라우저 공용 Web API, @types/node 불필요)
  ...
  return {
    ...
    tossCancel(paymentKey, reason, o) {
      const body: Record<string, unknown> = { cancelReason: (reason || "고객 요청").slice(0, 200) };
      if (typeof o.cancelAmount === "number" && o.cancelAmount > 0) body.cancelAmount = o.cancelAmount;
      return request(
        `/v1/payments/${encodeURIComponent(paymentKey)}/cancel`,
        { method: "POST", body, headers: { "Idempotency-Key": o.idempotencyKey } },
        DEFAULT_TIMEOUT_MS,
      );
    },
    tossGetPayment(paymentKey, o = {}) {
      return request(`/v1/payments/${encodeURIComponent(paymentKey)}`, { method: "GET" }, o.timeoutMs ?? 8_000);
    },
  };
}

/** 기본 인스턴스 — `configurePayments({ secretKey })` 의 키를 호출 시점에 읽는다. checkout-sync 와 `+server.ts` 가 쓴다. */
const defaultToss = createToss();

export const tossConfirm: TossApi["tossConfirm"] = (input) => defaultToss.tossConfirm(input);
export const tossCancel: TossApi["tossCancel"] = (paymentKey, reason, opts) => defaultToss.tossCancel(paymentKey, reason, opts);
export const tossGetPayment: TossApi["tossGetPayment"] = (paymentKey, opts) => defaultToss.tossGetPayment(paymentKey, opts);

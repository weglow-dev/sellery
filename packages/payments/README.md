...
| import | 어디서 |
|---|---|
| `@sellery/payments` · `@sellery/payments/money` · `@sellery/payments/checkout-rules` | **순수** — `+page.svelte` · `+page.server.ts` · vitest. `formatKRW` `generateOrderId` `ORDER_ID_PREFIX` · `parseCheckoutParams` `validateShipping` `parseSuccessParams` `failText` `failPageReason` … |
| `@sellery/payments/server/toss` · `@sellery/payments/server/checkout-sync` · `@sellery/payments/server/config` | **서버 전용** — 앱 `src/lib/server/payments.ts` 배럴을 통해서만 (`$lib/server/` 가드 + `scripts/check-boundaries.mjs`) |
...
src/money.ts                 formatKRW · generateOrderId · ORDER_ID_PREFIX

...
<script lang="ts">
	/**
	 * 서브내비 (ux-spec §2.2 · web sub-nav.tsx · 프로토타입 SCREENS.customer 순서): `진행 중인 판매`(/) · `인플루언서`(/influencers) · `내 주문`(/account/orders, 로그인 시만) · `주문 조회`(/orders/lookup, 로그아웃 시만 — 비회원 구매 0021) · `셀러리 소개`(/about) · 우측 persona.
	...
	const tabs = $derived([
		...
		...(user ? [{ href: '/account/orders', label: '내 주문' }] : [{ href: '/orders/lookup', label: '주문 조회' }]),

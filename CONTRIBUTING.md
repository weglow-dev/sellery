...
| 등급 · 샘플 · 포인트 상수 | `js/02-state.js` (`GRADES` `DATA_PRICE` `BG_DISC` `CELERY_PER` …) + `docs/*.md` |
...
- Vercel이 PR마다 **미리보기 URL**을 댓글로 달아줍니다. 병합 전에 거기서 눌러보세요. (연동 전이면 브랜치를 받아 로컬에서 확인)
...
(저장소 Settings → Secrets에 `ANTHROPIC_API_KEY` 가 등록돼 있고, 조직의 Claude GitHub 앱에 이 저장소가 추가돼 있어야 동작합니다.)
